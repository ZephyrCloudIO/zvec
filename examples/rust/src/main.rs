...
unsafe fn create_simple_test_collection(collection: &mut *mut ZVecCollection) -> ZVecErrorCode {
    ...
    let schema = zvec_collection_schema_create(schema_name.as_ptr());
    if schema.is_null() {
        return ZVecErrorCode_ZVEC_ERROR_INTERNAL_ERROR;
    }

    // Invert index params for string fields
    let invert_params = zvec_index_params_create(ZVecIndexType_ZVEC_INDEX_TYPE_INVERT);
    ...
    let hnsw_params = zvec_index_params_create(ZVecIndexType_ZVEC_INDEX_TYPE_HNSW);
    ...
    let id_field = zvec_field_schema_create(
        id_name.as_ptr(),
    ...
    let rc = zvec_collection_schema_add_field(schema, id_field);
    ...
    let text_field = zvec_field_schema_create(
        text_name.as_ptr(),
        ZVecDataType_ZVEC_DATA_TYPE_STRING,
        true,
        0,
    ...
    let rc = zvec_collection_schema_add_field(schema, text_field);
    if rc != ZVecErrorCode_ZVEC_OK {
        zvec_index_params_destroy(invert_params);
        zvec_index_params_destroy(hnsw_params);
        zvec_collection_schema_destroy(schema);
    ...
    let emb_field = zvec_field_schema_create(
        emb_name.as_ptr(),
        ZVecDataType_ZVEC_DATA_TYPE_VECTOR_FP32,
        false,
        3,
    );
    zvec_field_schema_set_hnsw_index(emb_field, hnsw_params);
    let rc = zvec_collection_schema_add_field(schema, emb_field);
    if rc != ZVecErrorCode_ZVEC_OK {
        zvec_index_params_destroy(invert_params);
        zvec_index_params_destroy(hnsw_params);
        zvec_collection_schema_destroy(schema);
    ...
    // Default options
    let options = zvec_collection_options_create();
    if options.is_null() {
        zvec_collection_schema_destroy(schema);
        return ZVecErrorCode_ZVEC_ERROR_RESOURCE_EXHAUSTED;
    }
    ...
    let rc = zvec_collection_create_and_open(path.as_ptr(), schema, options, collection);
